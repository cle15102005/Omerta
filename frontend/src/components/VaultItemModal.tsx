...
import { generateNameSalt } from '../crypto/merkle.crypto';
import { encryptPayload, decryptPayload, sha256hex } from '../crypto/vault.crypto';

import type { VaultItemDecrypted, VaultItemPayload, Category } from '../types';

interface VaultItemModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  existingItem?: VaultItemDecrypted;
}
...
export function VaultItemModal({ isOpen, onClose, onSuccess, existingItem }: VaultItemModalProps) {
  ...
  useEffect(() => {
    if (existingItem) {
      ...
      setName(existingItem.payload.name);
      ...
      if (existingItem.history && existingItem.history.length > 0 && pek && existingItem.category === 'password') {
        setIsDecryptingHistory(true);
        Promise.all(
          existingItem.history.map(async (h) => {
            try {
              const p = await decryptPayload(pek, h.encryptedData) as VaultItemPayload;
              return { password: p.password || '', savedAt: h.savedAt };
            } catch {
              return null;
            }
          })
        ).then(results => {
          setDecryptedHistory(results.filter(Boolean) as HistoryEntry[]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!pek) { toast.error('Encryption key missing. Please log in again.'); return; }
    if (!vaultIndex) { toast.error('Vault index missing. Please refresh the page.'); return; }
    
    setLoading(true);
    try {
      const nameSalt = existingItem ? existingItem.payload.nameSalt : generateNameSalt();
      const payload: VaultItemPayload = { name, nameSalt, username, password, url, apiKey, note, cardNumber, cardHolder, cardExpiry, cardCvv };
      
      const encryptedData = await encryptPayload(pek, payload);
      const nameLeafHash = await sha256hex(name + nameSalt);

      const newLeaves = [...vaultIndex.leaves];
      if (!existingItem) {
        newLeaves.push({ name, nameSalt, hash: nameLeafHash });
      } else {
        const idx = newLeaves.findIndex(l => l.hash === existingItem.nameLeafHash);
        if (idx !== -1) newLeaves[idx] = { name, nameSalt, hash: nameLeafHash };
      }
      ...
      const payloadDto = {
        ...
        category,
        encryptedData,
        vaultIndex: { leafHashes: newLeaves.map(l => l.hash), merkleRoot: '' }
      };

      if (existingItem) {
        await vaultApi.update(existingItem._id, payloadDto);
        toast.success('Item updated');
      } else {
        await vaultApi.create(payloadDto);
  ...
  const confirmDelete = async () => {
    ...
    setLoading(true);
    try {
      await vaultApi.delete(existingItem._id, { merkleRoot: '', leafHashes: [] });
          ...
          <div className="flex flex-col gap-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
            {[...decryptedHistory].reverse().map((h, idx) => (
              <div key={idx} className="flex items-center justify-between bg-black/20 p-2 rounded border border-white/5 hover:border-white/20 transition-colors">
                ...
                {isEditing && (
                  <Button type="button" variant="ghost" onClick={() => handleRestore(h.password)} className="text-xs flex items-center gap-1 text-accent hover:bg-accent/10">
  ...
  const handleRestore = (histPass: string) => {
    setPassword(histPass);
    toast.success('Restored previous password. Click Save to finalize.', { icon: '⏪' });
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={existingItem ? (isEditing ? 'Edit Item' : name) : 'New Item'}>
      {!isEditing ? (
        <div className="flex-col flex gap-5">
          ...
          {category === 'note' && (
            <div className="flex-col flex gap-1">
              ...
              <div className="text-base text-white whitespace-pre-wrap bg-surface p-4 rounded border border-dim">
                {note || 'Empty Note'}
              </div>
            </div>
          )}
          ...
          {category === 'card' && (
            <div className="flex flex-col gap-4 bg-surface p-4 rounded border border-dim">
              <div className="flex-col flex gap-1">
                <label className="text-xs text-dim uppercase font-semibold">Cardholder Name</label>
                <div className="text-base text-white uppercase tracking-wider">{cardHolder || 'N/A'}</div>
              </div>
              <div className="flex-col flex gap-1 mt-2">
                <label className="text-xs text-dim uppercase font-semibold">Card Number</label>
                <div className="font-mono text-lg text-white tracking-[0.15em] flex items-center justify-between">
                  <span>{mainPasswordRevealed ? cardNumber : `•••• •••• •••• ${cardNumber.slice(-4) || '••••'}`}</span>
                  <button type="button" onClick={() => setMainPasswordRevealed(!mainPasswordRevealed)} className="text-dim hover:text-white outline-none focus:outline-none" style={{ border: 'none', background: 'transparent', padding: 0, boxShadow: 'none' }}>
                    {mainPasswordRevealed ? <EyeOff size={18} /> : <Eye size={18} />}
                  </button>
                </div>
              ...
              <div className="flex justify-between mt-2">
                <div className="flex-col flex gap-1">
                  ...
                  <div className="font-mono text-base text-white">{cardExpiry || 'MM/YY'}</div>
                </div>
                <div className="flex-col flex gap-1">
                  <label className="text-xs text-dim uppercase font-semibold">CVV</label>
                  <div className="font-mono text-base text-white">{mainPasswordRevealed ? cardCvv : '•••'}</div>
                </div>
              </div>
            </div>
          )}
      ...
      <Modal isOpen={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)} title="Delete Item">
        <div className="flex flex-col gap-5">
          <p className="text-sm text-dim">Are you sure you want to permanently delete this item? This action cannot be undone.</p>
